...
directive cfv_consumer_directives.InspectorTabImplementation {
    ...
    debug_test_tab_guidance: {
        ...
        debug_section: {
            ...
            execution_controls: {
                run_flow_from_trigger: "Button to execute the entire `props.currentFlowFqn` using provided/generated trigger input. Calls `props.actions.simulateFlowExecution(props.currentFlowFqn, null, triggerInputData)`. CRITICAL: Execution results must update the visualizer nodes with execution status and data. Nodes should show SUCCESS/FAILURE/RUNNING states, NOT reset to 'not executed'. ENHANCED: Execution now uses advanced streaming with real-time Server-Sent Events, sophisticated dependency resolution, and natural timing based on component configuration. The server performs comprehensive dependency analysis, detects circular dependencies (treated as warnings), and uses layered execution with fallback strategies for deadlock resolution. Nodes start in PENDING state and progress through RUNNING to SUCCESS/FAILURE with proper React re-rendering through object reference management.",
                run_flow_up_to_step: "Button to simulate execution up to the `props.selectedElement` (if it's a step). Calls `props.actions.simulateFlowExecution(props.currentFlowFqn, props.selectedElement.id, triggerInputData)`. CRITICAL: Execution results must update the visualizer nodes with execution status and data. Nodes should show SUCCESS/FAILURE/RUNNING states, NOT reset to 'not executed'. ENHANCED: Supports advanced dependency-aware partial execution with proper step ordering and parallel execution for fork branches. The server analyzes dependencies and executes steps in optimal order while respecting the target step constraint.",
                ...
                execution_state_management: "All execution actions must propagate results back to the main visualizer to update node execution states. Nodes should start with no execution status in design mode and only show execution status after debug/test execution. Execution status should be SUCCESS, FAILURE, RUNNING, SKIPPED, or PENDING - never 'not executed'. ENHANCED: Advanced state management ensures proper React re-rendering through new object reference creation for all state updates. The client stream handler pre-populates all flow steps with PENDING status for immediate visual feedback and manages sophisticated error handling with automatic reconnection.",
                enhanced_streaming_features: "ENHANCED: Execution now uses Server-Sent Events for real-time streaming updates with comprehensive dependency analysis. The server performs advanced expression parsing to extract step dependencies from complex input mappings, detects circular dependencies (treated as warnings rather than fatal errors), and implements layered execution with multiple fallback strategies. Client-side stream handler ensures proper React re-rendering through object reference management and provides robust connection management with automatic reconnection.",
                dependency_resolution_features: "ENHANCED: The server execution engine now includes sophisticated dependency resolution with cycle detection using Depth-First Search algorithms, comprehensive expression parsing supporting complex JSON-like expressions with multiple step references, and layered execution strategy for optimal parallel processing. Circular dependencies are treated as warnings and execution continues with fallback strategies including independent step execution and deadlock-breaking mechanisms.",
                parallel_execution_support: "ENHANCED: Advanced parallel execution within dependency layers, with steps executing concurrently when dependencies are satisfied. Fork components automatically trigger parallel execution of downstream branches, and the execution engine maximizes parallelization opportunities while respecting dependency constraints. Error isolation ensures failed steps don't block independent execution paths."
            },
