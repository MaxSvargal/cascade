...
model cfv_models.UnifiedDebugTestActions {
    ...
    simulateFlowExecution: cfv_models.Function {
        required: true;
        description: "Simulate complete flow execution from trigger through all steps up to target step. Signature: (flowFqn: string, targetStepId: string, triggerData?: any) => Promise<FlowSimulationResult>";
    }
...
model cfv_models.FlowSimulationResult {
    description: "Result of simulating flow execution up to a target step."
    flowFqn: String { required: true; }
    targetStepId: String { required: true; }
    triggerData: cfv_models.Any { required: true; description: "Input data that triggered the simulation." }
    stepResults: Record<String, cfv_models.StepSimulationResult> { required: true; description: "Results for each step in execution order." }
    finalInputData: cfv_models.Any { required: true; description: "Resolved input data for the target step." }
    executionOrder: List<String> { required: true; description: "Order of step execution." }
    contextState: Record<String, cfv_models.Any> { required: true; description: "Final context state after simulation." }
    errors: List<String> { required: true; description: "Any errors encountered during simulation." }
}
